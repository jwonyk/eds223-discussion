...
```{r}
magpie <- read_csv(here("data", "magpie_obvs.csv"))
tule_elk <- read_csv(here("data", "tule_elk_obvs.csv"))
...
# Extract numeric suffix of filenames and convert to numeric
  order(as.numeric(gsub(".*_(\\d+)\\.tif$", "\\1", bioclim)))]
bioclim_rast <- rast(bioclim_sort)
```

```{r}
variables <- c("annualMeanTemp", "meanDiurnalRange", "isothermality", 
               "tempSeasonality", "maxTempWarmMonth", "maxTempColdMonth", 
               "tempAnnualRange", "meanTempWetQ", "meanTempDryQ", 
               "meanTempWarmQ", "meanTempColdQ", "annualPrecip", 
               "precipWetMonth", "precipDryMonth", "precipSeasonality", 
...
map_1 <- tm_shape(bioclim_crop[["annualMeanTemp"]]) +
...
map_2 <- tm_shape(bioclim_crop[["annualPrecip"]]) +
...
```{r}
plot_1 <- ggplot(data = bioClim_pts, 
                 aes(x = annualPrecip, y = annualMeanTemp)) +
...
plot_2 <- ggplot(data = bioClim_random_pts, 
                 aes(x = annualPrecip, y = annualMeanTemp)) +
  geom_point(shape = 16) +
  labs(x = "Annual precipitation",
       y = element_blank(), 
...
```{r}
climate_envelope <- function(clim_rast, clim_var1, 
                             clim_var2, occurences, species_name) {
  ...
  map_1 <- tm_shape(clim_crop[[clim_var1]]) +
    tm_raster(col.scale = tm_scale_continuous(values = "blue")) +
  ...
  plot_1 <- ggplot(data = clim_pts, 
                   aes_string(x = clim_var1, y = clim_var2)) +
    geom_point(shape = 16, color = "#3a5a40") +
    labs(title = paste("Species climate niche of", 
                       str_to_sentence(species_name))) +
    theme_bw()
  
  plot_2 <- ggplot(data = clim_random_pts, 
                   aes_string(x = clim_var1, y = clim_var2)) +
    geom_point(shape = 16) +
    labs(y = element_blank(), 
         title = paste("Background climate of", 
                       str_to_sentence(species_name))) + 
    theme_bw()
